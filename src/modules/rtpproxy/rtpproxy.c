	...
	for(;;) {
		...
		for(;;) {
			...
			if (c2p != NULL || !c1p_altered) {
                ...
                char *call_id;
                call_id = (char*)malloc(strlen(callid.s)+1);
                call_id = strcpy(call_id, callid.s);
                call_id = strtok(call_id, "@");
				if (msg->first_line.u.request.method_value == 1) {

                    invite(call_id, oldip.s, oldport.s, newip.strip.s, newport.s);
				}
				if (msg->first_line.u.request.method_value == 200) {
					ok200(call_id, oldip.s, oldport.s, newip.strip.s, newport.s);
				}
